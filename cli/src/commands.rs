...
async fn read_each<F>(ctx: &Ctx, targets: Vec<Resolved>, op: F) -> Result<Output>
where
    F: AsyncFn(&HttpTransport, &Resolved) -> Result<(Value, String)>,
{
    let t = ctx.transport();
    let mut results = Vec::with_capacity(targets.len());
    for r in &targets {
        results.push(match op(&t, r).await {
...
async fn read_cmd<F>(ctx: &Ctx, sel: &Selector, action: &str, op: F) -> Result<Output>
where
    F: AsyncFn(&HttpTransport, &Resolved) -> Result<(Value, String)>,
{
    ...
    read_each(ctx, targets, op).await
}
...
pub async fn status(ctx: &Ctx, sel: &Selector) -> Result<Output> {
    read_cmd(ctx, sel, "query status", async |t, r| {
        let s = ops::get_status(t, &r.addr).await?;
        Ok((to_value(&s), render::status(&s)))
    })
    .await
}

pub async fn power(ctx: &Ctx, sel: &Selector) -> Result<Output> {
    read_cmd(ctx, sel, "read power", async |t, r| {
        let s = ops::get_status(t, &r.addr).await?;
...
pub async fn energy(ctx: &Ctx, sel: &Selector) -> Result<Output> {
    read_cmd(ctx, sel, "read energy", async |t, r| {
        let s = ops::get_status(t, &r.addr).await?;
...
pub async fn health(ctx: &Ctx, sel: &Selector) -> Result<Output> {
    read_cmd(ctx, sel, "check health", async |t, r| {
        let mut s = ops::get_status(t, &r.addr).await?;
        ...
        match ops::mqtt_info(t, &r.addr).await {
...
pub async fn discover(ctx: &Ctx, range: Option<String>, concurrency: usize) -> Result<Output> {
...
pub async fn set_power(
    ...
    let mut results = Vec::with_capacity(targets.len());
    for r in &targets {
        results.push(match ops::set_power(&t, &r.addr, relay, action).await {
            Ok(state) => PerDevice {
                label: r.label.clone(),
                host: r.addr.host.clone(),
...
pub async fn config_get(ctx: &Ctx, sel: &Selector, setting: &str) -> Result<Output> {
    validate_setting(setting)?;
    let setting = setting.to_string();
    read_cmd(
        ctx,
        sel,
        &format!("read setting {setting}"),
        async move |t, r| {
            let v = ops::config_get(t, &r.addr, &setting).await?;
...
pub async fn config_set(ctx: &Ctx, sel: &Selector, setting: &str, value: &str) -> Result<Output> {
    validate_setting(setting)?;
    let targets = resolve(ctx, sel)?;
    if let Gate::Stop(out) = gate(
        ctx,
    ...
    let mut results = Vec::with_capacity(targets.len());
    for r in &targets {
        results.push(match ops::config_set(&t, &r.addr, setting, value).await {
            Ok(v) => PerDevice {
                label: r.label.clone(),
                host: r.addr.host.clone(),
                value: v.clone(),
...
pub async fn backup(ctx: &Ctx, sel: &Selector, out: Option<PathBuf>) -> Result<Output> {
    let targets = resolve(ctx, sel)?;
    ...
    for r in &targets {
        ...
        results.push(
            match ops::backup_config(&t, &r.addr).await.and_then(|bytes| {
...
pub async fn firmware_check(ctx: &Ctx, sel: &Selector) -> Result<Output> {
    read_cmd(ctx, sel, "check firmware version", async |t, r| {
        let version = ops::firmware_version(t, &r.addr).await?;
        Ok((
...
pub async fn firmware_update(ctx: &Ctx, sel: &Selector, url: Option<String>) -> Result<Output> {
    let targets = resolve(ctx, sel)?;
    ...
    let t = ctx.transport();
    for r in &targets {
        match ops::firmware_version(&t, &r.addr).await {
    ...
    for r in &targets {
        results.push(
            match ops::firmware_update(&t, &r.addr, url.as_deref()).await {
                Ok(_) => PerDevice {
                    label: r.label.clone(),
                    host: r.addr.host.clone(),
...
pub async fn console(ctx: &Ctx, sel: &Selector, command: &str) -> Result<Output> {
    let targets = resolve(ctx, sel)?;
    ...
    read_each(ctx, targets, async move |t, r| {
        let v = ops::console(t, &r.addr, &cmd).await?;
        Ok((v.clone(), format!("{}: {}", r.label, compact(&v))))
    })
    .await
}
...
pub async fn template_get(ctx: &Ctx, sel: &Selector) -> Result<Output> {
    read_cmd(ctx, sel, "read template", async |t, r| {
        let v = ops::template_get(t, &r.addr).await?;
        Ok((v.clone(), format!("{}: {}", r.label, compact(&v))))
    })
    .await
}

pub async fn template_apply(ctx: &Ctx, sel: &Selector, json_str: &str) -> Result<Output> {
    let targets = resolve(ctx, sel)?;
    ...
    let mut results = Vec::with_capacity(targets.len());
    for r in &targets {
        results.push(match ops::template_apply(&t, &r.addr, json_str).await {
            Ok(v) => PerDevice {
                label: r.label.clone(),
                host: r.addr.host.clone(),
                value: v.clone(),
...
pub async fn watch(ctx: &Ctx, sel: &Selector, interval: u64) -> Result<Output> {
    let targets = resolve(ctx, sel)?;
    if ctx.dry_run {
    ...
    loop {
        match ctx.format {
            ...
            OutputFormat::Text => {
                print!("\x1b[2J\x1b[H");
                for r in &targets {
                    match ops::get_status(&t, &r.addr).await {
                        Ok(s) => println!("{}\n", render::status(&s)),
            ...
            OutputFormat::Json => {
                let mut snapshot: Vec<Value> = Vec::with_capacity(targets.len());
                for r in &targets {
                    snapshot.push(match ops::get_status(&t, &r.addr).await {
                        Ok(s) => json!({
